...
import 'models/current_user.dart'; // Your QuickType model: Welcome
...
class _SpotifyProfilePageState extends State<SpotifyProfilePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: Colors.black, title: const Text("Spotify Profile", style: TextStyle(color: Colors.white))),
              ...
              child: Column(
                children: [
                  if (user.images.isNotEmpty)
                    CircleAvatar(radius: 60, backgroundImage: NetworkImage(user.images[0]["url"]))
                  else
                    const CircleAvatar(radius: 60, backgroundColor: Colors.grey, child: Icon(Icons.person, size: 60, color: Colors.white70)),
                  const SizedBox(height: 20),
                  Text(user.displayName, style: const TextStyle(color: Colors.white, fontSize: 26, fontWeight: FontWeight.bold)),
